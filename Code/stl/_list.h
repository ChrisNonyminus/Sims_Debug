...
protected:
  size_type buffer_size() { return list_node_allocator.init_page_size(); }
...
public:
  ...
  const_iterator begin() const {
    return (const_iterator((link_type)((*node).next)));
  }
...
template <class T> list<T>::buffer_pointer list<T>::buffer_list = 0;

template <class T> list<T>::link_type list<T>::free_list = 0;

template <class T> list<T>::link_type list<T>::next_avail = 0;

template <class T> list<T>::link_type list<T>::last = 0;

template <class T> list<T>::size_type list<T>::number_of_lists = 0;

template <class T>
list<T>::list_node_allocator_type list<T>::list_node_allocator;

template <class T> list<T>::value_allocator_type list<T>::value_allocator;

template <class T> list<T>::buffer_allocator_type list<T>::buffer_allocator;
...
template <class T> inline bool operator==(const list<T> &x, const list<T> &y) {
...
template <class T> void list<T>::deallocate_buffers() {
  while (buffer_list) {
...
template <class T> void list<T>::erase(iterator first, iterator last) {
  while (first != last)
    erase(first++);
}
