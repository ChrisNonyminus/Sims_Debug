...
template <class T> inline T __median(T a, T b, T c) {
  if (a < b)
    if (b < c)
      return b;
    else if (a < c)
...
__unique_copy(InputIterator first, InputIterator last,
              BidirectionalIterator result, bidirectional_iterator_tag) {
...
ForwardIterator __inplace_stable_partition(ForwardIterator first,
  ...
  ForwardIterator first_cut =
      __inplace_stable_partition(first, middle, pred, len / 2);
...
template <class ForwardIterator, class Pointer, class Predicate, class Distance,
          class T>
ForwardIterator
__stable_partition_adaptive(ForwardIterator first, ForwardIterator last,
  ...
  ForwardIterator second_cut =
      __stable_partition_adaptive(middle, last, pred, len - len / 2, buffer,
...
template <class ForwardIterator, class Predicate, class Pointer, class Distance>
...
__merge_aux(RandomAccessIterator1 first1, RandomAccessIterator1 last1,
            RandomAccessIterator2 first2, RandomAccessIterator2 last2,
            RandomAccessIterator3 result, Distance &fill_pointer, T *) {
  Distance len = 0;
...
void __merge_sort_loop(RandomAccessIterator1 first, RandomAccessIterator1 last,
  ...
  while (last - first >= two_step) {
    result = merge(first, first + step_size, first + step_size,
                   first + two_step, result);
    first += two_step;
  }
  step_size = min(Distance(last - first), step_size);
...
void __merge_sort_with_buffer(RandomAccessIterator first,
  ...
  while (step_size < len) {
    __merge_sort_loop_aux(first, last, buffer, step_size, fill_pointer, (T *)0);
    step_size *= 2;
...
void __stable_sort_adaptive(RandomAccessIterator first,
  ...
  if (len > buffer_size) {
    __stable_sort_adaptive(first, middle, buffer, buffer_size, fill_pointer,
                           (T *)0);
    __stable_sort_adaptive(middle, last, buffer, buffer_size, fill_pointer,
                           (T *)0);
  } else {
  ...
  __merge_adaptive(first, middle, last, Distance(middle - first),
                   Distance(last - middle), buffer, buffer_size, fill_pointer,
                   (T *)0);
}
...
void __stable_sort_adaptive(RandomAccessIterator first,
  ...
  if (len > buffer_size) {
    ...
    __merge_sort_with_buffer(first, middle, buffer, fill_pointer, (T *)0, comp);
...
inline void __stable_sort_aux(RandomAccessIterator first,
  ...
  __stable_sort(
      first, last,
      get_temporary_buffer(Distance(last - first), value_type(first)));
}
...
RandomAccessIterator __partial_sort_copy(InputIterator first,
  ...
  while (first != last) {
    if (*first < *result_first)
      __adjust_heap(result_first, Distance(0),
                    Distance(result_real_last - result_first), T(*first), comp);
    first++;
  }
...
__lower_bound(ForwardIterator first, ForwardIterator last, const T &value,
...
template <class ForwardIterator, class T, class Compare, class Distance>
inline ForwardIterator
__upper_bound(ForwardIterator first, ForwardIterator last, const T &value,
              Compare comp, Distance *, bidirectional_iterator_tag) {
...
void __merge_adaptive(BidirectionalIterator first, BidirectionalIterator middle,
  ...
  if (len1 <= len2 && len1 <= buffer_size) {
    ...
    __merge_adaptive(new_middle, second_cut, last, len1 - len11, len2 - len22,
                     buffer, buffer_size, fill_pointer, (T *)0);
  }
}
...
void __merge_adaptive(BidirectionalIterator first, BidirectionalIterator middle,
  ...
  if (len1 <= len2 && len1 <= buffer_size) {
    ...
    __merge_adaptive(new_middle, second_cut, last, len1 - len11, len2 - len22,
                     buffer, buffer_size, fill_pointer, (T *)0, comp);
  }
}
...
__inplace_merge_aux(BidirectionalIterator first, BidirectionalIterator middle,
                    BidirectionalIterator last, Distance *, Compare comp) {
  Distance len1 = 0;
  distance(first, middle, len1);
  Distance len2 = 0;
  distance(middle, last, len2);
  __inplace_merge(first, middle, last, len1, len2,
                  get_temporary_buffer(len1 + len2, value_type(first)), comp);
}
