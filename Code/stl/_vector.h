...
public:
  ...
  const_iterator end() const {
    const_iterator it(v.end());
    return (it);
  }
  reverse_iterator rbegin() {
    reverse_iterator it(v.rbegin());
    return (it);
  }
  ...
  reverse_iterator rend() {
    reverse_iterator it(v.rend());
    return (it);
  }
  const_reverse_iterator rend() const {
    const_reverse_iterator it(v.rend());
    return (it);
  }
  ...
  vector(size_type n, const T &value = T()) {
    for (int i = 0; i < n; i++)
      v.push_back((void *)new T(value));
  }
  vector(const vector<T> &x) {
    for (int i = 0; i < n; i++)
      v.push_back(new T(x[i]));
  }
...
template <class T>
inline bool operator==(const vector<T>::iterator &x,
                       const vector<T>::iterator &y) {
  return (x.itr == y.itr);
}
...
template <class T>
inline bool operator==(const vector<T>::reverse_iterator &x,
                       const vector<T>::reverse_iterator &y) {
  return (x.itr == y.itr);
}
...
template <class T> void vector<T>::insert_aux(iterator position, const T &x) {
  if (finish != end_of_storage) {
