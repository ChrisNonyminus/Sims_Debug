...
public:
  ...
  public:
    ...
    const_reference operator[](difference_type n) { return *(*this + n); }
...
public:
  ...
  deque(const deque<T> &x) : start(), finish(), length(0), map(0), map_size(0) {
    buffer_size = data_allocator.init_page_size();
...
template <class T> deque<T>::size_type deque<T>::buffer_size = 0;
// should be data_allocator.init_page_size(); // Borland bug

template <class T> bool operator==(const deque<T> &x, const deque<T> &y) {
...
template <class T> void deque<T>::allocate_at_begin() {
...
template <class T> void deque<T>::allocate_at_end() {
  pointer p = data_allocator.allocate(buffer_size);
  if (!empty()) {
...
template <class T> void deque<T>::deallocate_at_begin() {
...
template <class T> void deque<T>::deallocate_at_end() {
...
template <class T> void deque<T>::erase(iterator position) {
  if (end() - position > position - begin()) {
...
template <class T> void deque<T>::erase(iterator first, iterator last) {
