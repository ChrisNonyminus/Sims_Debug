...
protected:
  ...
  link_type get_node() {
    ...
    return free_list ? (free_list = (link_type)(free_list->right_link), tmp)
...
public:
  ...
  class const_iterator : public bidirectional_iterator<Value, difference_type> {
    friend class rb_tree<Key, Value, KeyOfValue, Compare>;
...
public:
  ...
  rb_tree(const Compare &comp = Compare(), bool always = true) : node_count(0) {
    key_compare = comp;
  ...
  rb_tree(const rb_tree<Key, Value, KeyOfValue, Compare> &x, bool always = true)
    ...
    insert_always = always;
    init();
    for (rb_tree<Key, Value, KeyOfValue, Compare>::const_iterator i = x.begin();
         i != x.end(); ++i)
      insert(end(), *i);
  }
  ~rb_tree() {
...
rb_tree<Key, Value, KeyOfValue, Compare>::__insert(link_type x, link_type y,
                                                   const Value &v) {
  ++node_count;
