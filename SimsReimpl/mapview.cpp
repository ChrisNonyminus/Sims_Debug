...
MapViewOfFileClass::MapViewOfFileClass(const wchar_t *fileName)
    : m_hFile(INVALID_HANDLE_VALUE), m_hFileMapping(NULL), m_lpFileBase(NULL),
      m_dosHeader(NULL), m_ntHeader(NULL), m_optionalHeader(NULL),
  ...
  if (m_hFile != INVALID_HANDLE_VALUE) {
    m_hFileMapping =
        CreateFileMapping(m_hFile, NULL, PAGE_READONLY, 0, 0, NULL);
    ...
    if (m_hFileMapping != NULL) {
      ...
      if (m_lpFileBase != NULL) {
        ...
        if (m_dosHeader->e_magic == IMAGE_DOS_SIGNATURE) {
          m_ntHeader = (PIMAGE_NT_HEADERS)((unsigned char *)m_dosHeader +
                                           m_dosHeader->e_lfanew);

          if (m_ntHeader->Signature == IMAGE_NT_SIGNATURE) {
            m_optionalHeader =
                (PIMAGE_OPTIONAL_HEADER)&m_ntHeader->OptionalHeader;
...
bool GetModuleSectionInfo(ImageSectionInfo &info) {
  ...
  if (GetModuleFileNameW(NULL, fileName, ARRAY_SIZE(fileName)) != 0) {
    ...
    if (optionalHeader != NULL) {
      info.BaseOfCode =
          LPVOID(optionalHeader->ImageBase + optionalHeader->BaseOfCode);
      info.BaseOfData = LPVOID(optionalHeader->BaseOfData);
      info.SizeOfCode = SIZE_T(optionalHeader->SizeOfCode);
      info.SizeOfData = SIZE_T(optionalHeader->SizeOfInitializedData +
